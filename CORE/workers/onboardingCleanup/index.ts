...
export const runOnboardingCleanup = async (): Promise<void> => {
  ...
  await Promise.all(
    legacyIncompleteUsers.map(async (user) => {
      try {
...
export const startOnboardingCleanupWorker = (): void => {
  if (intervalRef) {
    return;
  }
  ...
  logger.info(
    { intervalMs: config.onboarding.cleanupIntervalMs, ttlSeconds: config.onboarding.ttlSeconds },
    'Onboarding cleanup worker started',
  );
};

export const stopOnboardingCleanupWorker = (): void => {
  if (intervalRef) {
