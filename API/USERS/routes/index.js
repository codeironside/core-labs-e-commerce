...
usersRouter.put('/me/push-token', async (req, res, next) => {
    try {
        const { userId } = req.user;
        const payload = UpdatePushTokenSchema.parse(req.body);
        const user = await UserModel.findByIdAndUpdate(userId, { $set: { fcmToken: payload.fcmToken } }, { new: true }).select('-passwordHash +fcmToken');
        if (!user)
            throw new AppError(HTTP_STATUS.NOT_FOUND, MESSAGES.USER.NOT_FOUND);
...
usersRouter.delete('/me/push-token', async (req, res, next) => {
    try {
        const { userId } = req.user;
