...
import type { OnboardingDraft, OnboardingState } from '@api/USERS/interfaces/onboarding';
import { AppError } from '@core/middleware/errorHandler';
import { HTTP_STATUS } from '@core/constants';

const formatAddress = (details: NonNullable<OnboardingDraft['addressDetails']>): string =>
  [details.line1, details.line2, details.city, details.state, details.postalCode, details.country]
    .filter(Boolean)
    .join(', ');

import type { OnboardingStep } from '@core/constants/onboarding';

export const getNextStepForUserType = (
  userType: OnboardingDraft['userType'],
...
export const assertOnboardingReadyToComplete = (state: OnboardingState): void => {
  const { draft } = state;
...
const resolveIdentityStatus = (
  draft: OnboardingDraft,
): 'pending' | 'verified' | 'skipped' | 'admin_bypass' => {
  if (draft.identityVerificationStatus === 'admin_bypass') return 'admin_bypass';
  if (draft.identitySkipped) return 'skipped';
  if (draft.identityVerification?.ninVerified) return 'verified';
...
export const buildVendorProfileFromDraft = (draft: OnboardingDraft): Record<string, unknown> => {
