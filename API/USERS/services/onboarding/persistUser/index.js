...
import { findUserByEmail, findUserById, purgeIncompleteUsersByEmail, } from '@core/services/db/userLookup';
const purgeOrphanCredentialAccounts = async (email) => {
...
export const persistOnboardingUser = async (state) => {
    ...
    if (account.authMethod === 'credential' && account.passwordHash) {
        await AccountModel.findOneAndUpdate({ providerId: 'credential', accountId: normalizedEmail }, {
            userId: user._id,
            providerId: 'credential',
            accountId: normalizedEmail,
        ...
        await AccountModel.findOneAndUpdate({ providerId: account.oauthProvider, accountId: account.oauthProviderId }, {
            userId: user._id,
    ...
    await publishKafkaEvent(KAFKA_TOPICS.USER_REGISTERED, {
        userId,
