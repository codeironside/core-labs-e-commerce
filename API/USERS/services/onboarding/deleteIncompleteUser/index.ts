...
export const purgeLegacyIncompleteMongoUser = async (email: string): Promise<void> => {
  await purgeIncompleteUsersByEmail(email);
  logger.info({ email }, 'Incomplete users purged from canonical and legacy collections');
};

export const expireOnboardingSession = async (email: string): Promise<boolean> => {
  ...
  await publishKafkaEvent(
    KAFKA_TOPICS.ONBOARDING_EXPIRED,
    { userId: session.state.userId, email: session.state.account.email },
...
export const deleteIncompleteUser = async (userId: string): Promise<boolean> => {
