import { OnboardingAddressStepSchema, OnboardingCategoriesStepSchema, OnboardingExperienceStepSchema, OnboardingGuidelinesStepSchema, OnboardingIdentityStepSchema, OnboardingPayoutDetailsStepSchema, OnboardingPayoutStepSchema, OnboardingSocialStepSchema, OnboardingSubcategoriesStepSchema, } from '@api/USERS/schemas/onboarding';
...
const namesMatchRegisteredAccount = (registeredName, firstName, lastName) => {
...
const saveVendorPatch = async (token, patch, currentStep) => {
    const nextStep = getVendorNextStep(currentStep);
    return saveOnboardingDraft(token, patch, nextStep);
};
export const handleVendorWizardStep = async (step, body) => {
    if (step === ONBOARDING_STEPS.GUIDELINES) {
        ...
        const updated = await saveVendorPatch(payload.onboardingToken, {
    ...
    if (step === ONBOARDING_STEPS.CATEGORIES) {
        ...
        const state = await requireState(payload.onboardingToken);
        const updated = await saveVendorPatch(payload.onboardingToken, { categories: payload.categories, userType: 'vendor' }, ONBOARDING_STEPS.CATEGORIES);
        await publishStepSaved(state.userId, step);
        return { currentStep: updated.currentStep, draft: updated.draft };
    }
    if (step === ONBOARDING_STEPS.SUBCATEGORIES) {
        ...
        const state = await requireState(payload.onboardingToken);
        const updated = await saveVendorPatch(payload.onboardingToken, { subcategories: payload.subcategories }, ONBOARDING_STEPS.SUBCATEGORIES);
        await publishStepSaved(state.userId, step);
        return { currentStep: updated.currentStep, draft: updated.draft };
    }
    ...
    if (step === ONBOARDING_STEPS.SOCIAL) {
        ...
        const state = await requireState(payload.onboardingToken);
        const updated = await saveVendorPatch(payload.onboardingToken, { socialChannels: payload.socialChannels }, ONBOARDING_STEPS.SOCIAL);
        await publishStepSaved(state.userId, step);
        return { currentStep: updated.currentStep, draft: updated.draft };
    }
    ...
    if (step === ONBOARDING_STEPS.PAYOUT_DETAILS) {
        ...
        if (payoutMethod === 'bank') {
            ...
            const accountHolderName = state.draft.addressDetails?.fullName?.trim() || state.account.name;
            ...
            const updated = await saveVendorPatch(payload.onboardingToken, { bankDetails: resolvedBank }, ONBOARDING_STEPS.PAYOUT_DETAILS);
            await publishStepSaved(state.userId, step);
            return { currentStep: updated.currentStep, draft: updated.draft };
        }
        ...
        const updated = await saveVendorPatch(payload.onboardingToken, { cryptoWalletAddress: payload.cryptoWalletAddress }, ONBOARDING_STEPS.PAYOUT_DETAILS);
        await publishStepSaved(state.userId, step);
        return { currentStep: updated.currentStep, draft: updated.draft };
    }
    if (step === ONBOARDING_STEPS.IDENTITY) {
        ...
        if (payload.skipIdentity) {
            const updated = await saveVendorPatch(payload.onboardingToken, { identitySkipped: true, identityVerificationStatus: 'skipped' }, ONBOARDING_STEPS.IDENTITY);
            await publishStepSaved(state.userId, step);
            return { currentStep: updated.currentStep, draft: updated.draft };
        }
        ...
        if (identity.documentType === 'nin') {
            ...
            if (!namesMatchRegisteredAccount(state.account.name, ninResult.firstName, ninResult.lastName)) {
                throw new AppError(HTTP_STATUS.BAD_REQUEST, 'NIN identity does not match your registered account name.');
            }
            const updated = await saveVendorPatch(payload.onboardingToken, {
