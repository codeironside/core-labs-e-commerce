...
import type { OnboardingBankDraft } from '@api/USERS/interfaces/onboarding';
import {
  findBankNameByCode,
  resolveNigerianBankAccount,
} from '@core/services/paystack';

const accountNameMatchesHolder = (accountName: string, accountHolderName: string): boolean => {
...
export const buildDevBypassBankDetails = async (
  bankDetails: { bankCode: string; accountNumber: string },
  accountHolderName: string,
): Promise<OnboardingBankDraft> => {
...
export const resolveVendorBankDetails = async (
  bankDetails: { bankCode: string; accountNumber: string },
  accountHolderName: string,
): Promise<OnboardingBankDraft> => {
  ...
  if (config.env !== 'development' && !accountNameMatchesHolder(accountName, accountHolderName)) {
    throw new AppError(
      ...
      'Bank account name does not match the legal name on your profile. Use the same full name from the address step.',
    );
  }
