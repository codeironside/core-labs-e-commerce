import { OnboardingBasicStepSchema, OnboardingVendorBankStepSchema, OnboardingVendorProfileStepSchema, } from '@api/USERS/schemas/onboarding';
...
const vendorWizardSteps = new Set([
...
export const saveOnboardingStep = async (req, res, next) => {
    try {
        ...
        if (step === ONBOARDING_STEPS.BASIC) {
            ...
            if (payload.userType === ONBOARDING_USER_TYPES.VENDOR &&
                state.account.signupRole !== 'vendor') {
                throw new AppError(HTTP_STATUS.FORBIDDEN, 'Vendor accounts must be created through seller signup.');
            }
            ...
            const updated = await saveOnboardingDraft(payload.onboardingToken, {
            ...
            await publishKafkaEvent(KAFKA_TOPICS.ONBOARDING_STEP_SAVED, { userId: state.userId, step: ONBOARDING_STEPS.BASIC, userType: payload.userType }, state.userId);
            res.status(HTTP_STATUS.OK).json({
                success: true,
                message: 'Onboarding step saved.',
        ...
        if (step === ONBOARDING_STEPS.VENDOR_PROFILE) {
            ...
            await publishKafkaEvent(KAFKA_TOPICS.ONBOARDING_STEP_SAVED, { userId: state.userId, step: ONBOARDING_STEPS.VENDOR_PROFILE }, state.userId);
            res.status(HTTP_STATUS.OK).json({
                success: true,
        ...
        if (step === ONBOARDING_STEPS.VENDOR_BANK) {
            ...
            const resolvedBank = await resolveVendorBankDetails(payload.bankDetails, state.draft.addressDetails?.fullName?.trim() || state.account.name);
            const updated = await saveOnboardingDraft(payload.onboardingToken, { bankDetails: resolvedBank, payoutMethod: 'bank' }, ONBOARDING_STEPS.COMPLETE);
            await publishKafkaEvent(KAFKA_TOPICS.ONBOARDING_STEP_SAVED, { userId: state.userId, step: ONBOARDING_STEPS.VENDOR_BANK }, state.userId);
            res.status(HTTP_STATUS.OK).json({
                success: true,
