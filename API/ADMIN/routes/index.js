...
import { authenticate, requireRole } from '@core/middleware/rbac';
...
import { z } from 'zod';
import { getPlatformSettings, updatePlatformSettings, } from '@api/ADMIN/services/platform_settings';
import { fetchEngagementAnalytics } from '@api/ADMIN/services/engagementAnalytics';
import { assertCanManageTargetUser, buildAdminUserListFilter, buildAssignableRoles, } from '@api/ADMIN/services/assertCanManageUser';
import { activateVendorLivestream, banUserFromLivestreams, banUserFromPlatform, listVendorsForAdmin, unbanUserFromLivestreams, unbanUserFromPlatform, } from '@api/ADMIN/services/moderation';
    ...
    try {
        ...
        const [users, total] = await Promise.all([
            UserModel.find(listFilter)
                .select('name email role userType emailVerified createdAt platformBanned livestreamBanned vendorProfile.canGoLive')
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .lean(),
...
adminRouter.post('/users/:userId/livestream-ban', async (req, res, next) => {
    try {
        const actor = req.user;
...
adminRouter.post('/users/:userId/livestream-unban', async (req, res, next) => {
    try {
        const actor = req.user;
...
adminRouter.post('/vendors/:userId/activate-livestream', async (req, res, next) => {
    try {
        const actor = req.user;
        const { vendorPhotoUrl } = ActivateVendorLivestreamSchema.parse(req.body);
        const result = await activateVendorLivestream(actor.userId, actor.role, String(req.params.userId ?? ''), vendorPhotoUrl);
        res.status(HTTP_STATUS.OK).json({
            success: true,
...
adminRouter.get('/platform-settings', async (req, res, next) => {
    try {
...
adminRouter.post('/vendors/:userId/identity-bypass', async (req, res, next) => {
    try {
        const actor = req.user;
...
adminRouter.get('/engagement/analytics', async (_req, res, next) => {
    try {
...
adminRouter.get('/platform-analytics', async (req, res, next) => {
    try {
        const actor = req.user;
