...
import { createPrivySolanaWalletForUser, } from '@core/services/privy';
...
export const attachPrivyWalletToUser = async (opts) => {
    ...
    if (privyWallet.privyUserId) {
        await AccountModel.findOneAndUpdate({ providerId: 'privy', accountId: privyWallet.privyUserId }, {
    ...
    if (privyWallet.walletAddress) {
        await publishKafkaEvent(KAFKA_TOPICS.WALLET_PROVISIONED, {
            userId: opts.userId,
